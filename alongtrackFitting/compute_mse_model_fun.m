...
window_end_day = window_start_day + window_size-1;
...
if window_end_day > max(alongtrack.t)
    ...
    window_start_day = max(t0, window_end_day - window_size + 1);
end
windowCenters(i) = (window_start_day + window_end_day) / 2;
...
case 'Gaussian'
    ...
    [paramsCell, initParamsCell] = FitAlongTrackXYToEddyModelWindowed(alongtrack_window, eddyFit_fun, initParams, eddyPath_window, it_options,window=window_size);
...
getModelForDay = @(day) createInterpolatedModel(day, windowCenters, ssh_model);
