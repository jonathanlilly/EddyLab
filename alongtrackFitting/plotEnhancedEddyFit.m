function plotEnhancedEddyFit(alongtrack, eddyParams, paramsCell, trueParamsCell)
    ...
    for j = 1:totalTimeWindows
        ...
        % Calculate average and final values for true parameters
        days_in_window = unique(params.elapsed_time) + params.t0;
        
        % True positions through window
        t_points = params.elapsed_time;
        xo_true_full = eddyParams.xo(days_in_window);
        yo_true_full = eddyParams.yo(days_in_window);
        
        % True amplitudes and length scales
        A_true_full = eddyParams.A(1:length(days_in_window));
        L_true_full = eddyParams.L(1:length(days_in_window));
        
        % Fit positions through window
        xo_fit_full = params.x0 + params.cx * t_points;
        yo_fit_full = params.y0 + params.cy * t_points;
    ...
    for j = 1:totalTimeWindows
        ...
        window_times = params.elapsed_time + params.t0;
        
        % Extract parameters
        A_true_values = trueParams.A(1:length(window_times));
        L_true_values = trueParams.L(1:length(window_times));
        
        % Store window index for each time point for coloring
        window_idx = j * ones(size(window_times));
        
        % Accumulate data
        all_times = [all_times, window_times];
        ...
        plot([min(window_times), max(window_times)], [params.A*100, params.A*100], '-', 'Color', colors(j,:), 'LineWidth', 2);
        plot([min(window_times), max(window_times)], [params.L/1e3, params.L/1e3], '--', 'Color', colors(j,:), 'LineWidth', 2);
    end
...
function plotParameterEvolution(paramsCell, trueParamsCell)
    ...
    % Add overall title
    sgtitle('Evolution of Eddy Parameters', 'FontName', 'Times', 'FontSize', 16, 'FontWeight', 'bold');
end
...
function visualizeModelDataMisfit(alongtrack, eddyPath_fun_t, paramsCell, trueParamsCell, window_index)
    ...
    % Extract window data
    window_days = params.elapsed_time + params.t0;
    [alongtrack_window, ~] = extractAlongtrackWindow(alongtrack, window_days(1), window_days(end));
    ...
    for i = 1:length(alongtrack_window.ssh)
        t_idx = find(window_days == alongtrack_window.t(i));
        if ~isempty(t_idx)
            t_elapsed = params.elapsed_time(t_idx);
...
% Example usage:
% plotEnhancedEddyFit(alongtrack, eddyPath_fun_t, paramsCell, trueParamsCell);
% plotParameterEvolution(paramsCell, trueParamsCell);
% visualizeModelDataMisfit(alongtrack, eddyPath_fun_t, paramsCell, trueParamsCell, 3);