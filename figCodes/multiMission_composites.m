...
%%
%Alongtrack
[mz, xmid, ymid, ~, stdz] = composite2D(currentMission, eddyPath_fun_t,showplot=0);
[mzxy, rmid, ~, stdz_rt] = radialProfile(currentMission, eddyPath_fun_t,showplot=0);
...
row=1;
%%
figname='multiMission_composite';
plot_data = data;
%% subplots
main_fig = figure('color','w');
set(gcf,'Units','centimeters');  %Ensure all dimensions are in cm
set(gcf,'Position',[0 0 18 7.8]);  %Size of the figure. The first two numbers indicate the location of the lower left-hand corner.
...
% Create axes for the colorbars at the top
    cbar1_pos = [1.5, 5.3, 4.3, 1.5];
    cbar2_pos = [6.8, 5.3, 4.3, 1.5];
    legend_pos = [12.8, 5.3, 4.3, 1.5];
