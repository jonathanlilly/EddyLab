function[psip,psi,psif] = eddyFrameStreamFunction(xp,yp,ssh,uo,vo,f0)
%eddyFrameStreamFunction  Streamfunction of an eddy in the eddy frame.
%
%   PSIP = eddyFrameStreamFunction(XP,YP,SSH,UO,VO,F0) finds the 
...
%   are all m, UO and VO are in m/s, and F0 has units of 1/s. The 
...
%  
%   Usage: psip = eddyFrameStreamFunction(xp,yp,ssh,uo,vo,f0);
%          [psip,psi,psif] = eddyFrameStreamFunction(xp,yp,ssh,uo,vo,f0);
...
psif = zeros(length(xp),length(yp),length(uo));
