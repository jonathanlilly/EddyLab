...
%
%   [XP,YP,ZP1,ZP2,...,ZPN] = fieldInEddyFrame(X,Y,XO,YO,L,Z1,Z2,...,ZN) 
...
%   rows generated by EDDYCENTERFROMFIELD. The third dimension of Z is 
%   interpreted as time. 
%
...
%   interpolation method, with valid choices specified in INTERPN. The
%   default choice is method = "linear". 
% 
%   Usage: [xp,yp,zp] = fieldInEddyFrame(x,y,xo,yo,L,z);
%          [xp,yp,zp] = fieldInEddyFrame(x,y,xo,yo,L,z,dx = 2);
%          [xp,yp,zp] = fieldInEddyFrame(x,y,xo,yo,L,z,method = "spline");
...
for l=1:length(fieldIn)
    for k = 1:size(fieldIn{l},3)
        fieldOut{l}(:,:,k) = interpn(x,y,fieldIn{l}(:,:,k),xg+xo(k),yg+yo(k),options.Method);
    end
end
